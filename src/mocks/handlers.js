...
export const handlers = {
  getCharacter200: http.get("https://the-one-api.dev/v2/character", async () => {
    await delay(1000);
    return HttpResponse.json(getCharacter, { status: 200 });
  }),
  getCharacter500: http.get("https://the-one-api.dev/v2/character", async () => {
    await delay();
    return HttpResponse(null, { status: 500 });
  }),
  getPokemon200Hateoas: http.get(`${apiBaseUrlPokemon}pokemon`, async ({ request }) => {
    const url = new URL(request.url);
    const limit = Number(url.searchParams.get("limit")) || 10;
    const offset = Number(url.searchParams.get("offset"));
    const modifyGetPokemonAll = structuredClone(getPokemonAll);
    console.log(modifyGetPokemonAll);
    modifyGetPokemonAll.results = modifyGetPokemonAll.results.slice(
      offset,
      limit === 0 ? getPokemonAll.length : offset + limit
    );
    ...
    modifyGetPokemonAll.previous =
      processedPreviousOffset >= 0
        ? `${apiBaseUrlPokemon}pokemon/?offset=${processedPreviousOffset}&limit=${limit}`
        : null;
    console.log(modifyGetPokemonAll);
    await delay();
